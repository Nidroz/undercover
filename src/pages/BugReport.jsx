...
const REPO_URL = "https://github.com/your-username/undercover";
...
export default function BugReport({ inline = false, hide = false }) {
  ...
  // inline mode — compact button sitting above the chat bar
  const trigger = inline ? (
    <button
      ...
      onMouseEnter={e => e.currentTarget.style.color = "rgba(238,234,227,0.6)"}
      onMouseLeave={e => e.currentTarget.style.color = "rgba(238,234,227,0.3)"}
    >
      🐛 Report a bug
    </button>
  ) : (
    // floating mode — all other pages
    <button
      onClick={() => setOpen(true)}
      style={{
        position: "fixed",
    ...
    >
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 22, letterSpacing: 1.5, color: "#eeeae3" }}>
            🐛 Report a bug
          </span>
          <button onClick={() => setOpen(false)} style={{ background: "none", border: "none", color: "rgba(238,234,227,0.4)", fontSize: 20, cursor: "pointer", lineHeight: 1 }}>×</button>
        </div>
        <input
