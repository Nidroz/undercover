...
import GameViewTable from "./GameViewTable";
import GameViewStage from "./GameViewStage";
...
export default function Game() {
  ...
  const { view, setView, isDesktop } = useViewPreference();
  ...
  // shared props for alternative views
  const viewProps = {
  ...
  if (isDesktop && view === "table") return <GameViewTable {...viewProps} />;
  if (isDesktop && view === "stage") return <GameViewStage {...viewProps} />;

  // ── CHAT VIEW (default + mobile) ──
  return (
    <div className="game-layout">
      <div className="game-topbar">
        ...
        <div style={{ position: "relative" }}>
          <button className="btn-icon" onClick={() => setWordVisible(v => !v)} style={{ fontSize: 13 }}>
            👁️ My word
          </button>
          {wordVisible && (
            <div style={{
              ...
              <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 26, letterSpacing: 2, color: "var(--white)" }}>
                {myPlayer?.role === "mrWhite" ? "???" : myPlayer?.word}
              </span>
              <button className="btn-ghost" style={{ fontSize: 12, padding: "4px 10px" }} onClick={() => setWordVisible(false)}>
                Close
              </button>
            </div>
          )}
        ...
        {isDesktop && <ViewSwitcher view={view} setView={setView} />}
        {isHost && <button className="btn-vote-trigger" onClick={goToVote}>🗳️ Vote</button>}
      </div>
